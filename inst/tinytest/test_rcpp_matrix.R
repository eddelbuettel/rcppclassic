...
expect_equal(funx(x=M), list(dim1=2, dim2=3, rows=2, cols=3, p22=5, m=M), info = "RcppMatrix.int")


#test.RcppMatrix.double <- function() {
funx <- RcppMatrix_double
M <- matrix(1:6,2,3,byrow=TRUE)
expect_equal(funx(x=M), list(dim1=2, dim2=3, rows=2, cols=3, p22=5, m=M), info = "RcppMatrix.double")
...
expect_equal(funx(x=matrix(1:6,2,3,byrow=TRUE)),
...
expect_equal(funx(x=matrix(1.0*(1:6),2,3,byrow=TRUE)),
             list(dim1=2, dim2=3, rows=2, cols=3, p22=5),
...
funx <- RcppVector_int
expect_equal(funx(x=c(1:6)), list(size=6, p2=2, v=c(1:6)), info="RcppVector.int")

#test.RcppVector.double <- function() {
funx <- RcppVector_double
expect_equal(funx(x=c(1:6)), list(size=6, p2=2, v=c(1:6)), info="RcppVector.double")
...
x[2] <- NA
x[4] <- NaN
expect_equal(funx(x=x), list(na_2=1, na_3=0, nan_4=1, nan_5=0), info = "RcppMatrix.double.na.nan")

#test.RcppVectorView.int <- function() {
funx <- RcppVectorView_int
expect_equal(funx(x=c(1:6)), list(size=6, p2=2), info="RcppVectorView.int")

#test.RcppVectorView.double <- function() {
funx <- RcppVectorView_double
expect_equal(funx(x=1.0*c(1:6)), list(size=6, p2=2), info="RcppVectorView.double")
